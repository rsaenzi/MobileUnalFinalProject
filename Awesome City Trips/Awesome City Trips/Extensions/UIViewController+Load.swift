...
extension UIViewController {
    ...
    /**
     This function works if the UIViewController class name, the Storyboard file name,
    ...
    /**
     Loads the initial UIViewController in the storyboard. This function works if the
...
extension UIStoryboard {
    ...
    static func load<T: UIViewController>(_ viewControllerId: UIViewControllerId, from storyboardId: UIStoryboardId) -> T {
        
        let storyboard = UIStoryboard(name: storyboardId.rawValue, bundle: .main)
        
        guard let screen = storyboard.instantiateViewController(withIdentifier: viewControllerId.rawValue) as? T else {
            fatalError("No UIViewController \(viewControllerId) was found on storyboard \(storyboardId)")
        }
        return screen
    }
    
    static func loadInitialViewController<T: UIViewController>(from storyboardId: UIStoryboardId) -> T {
        
        let storyboard = UIStoryboard(name: storyboardId.rawValue, bundle: .main)
        
        guard let screen = storyboard.instantiateInitialViewController() as? T else {
            fatalError("No initial UIViewController on storyboard was found \(storyboardId)")
        }
        return screen
    }
}

enum UIStoryboardId: String {
    case login = "LoginStoryboard"
}

enum UIViewControllerId: String {
