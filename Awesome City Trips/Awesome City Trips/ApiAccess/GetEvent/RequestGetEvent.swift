...
typealias CompletionGetEvent = (_ response: ResponseGetEvent) -> Void

class RequestGetEvent {
    
    private init() {}
    
    static func request(eventId: Int64, completion callback: @escaping CompletionGetEvent) {
        
        let endpoint = ApiEndpoint.getEvent(eventId: eventId)
    ...
    private static func handleResponse(_ resultData: Response, _ callback: @escaping CompletionGetEvent) {
    ...
    private static func handleSuccess(_ jsonString: String, _ callback: @escaping CompletionGetEvent) {
        
        // Converts the jsonString into a valid Object
        guard let output: OutputGetEvent = jsonString.decodeFrom() else {
    ...
    private static func call(_ callback: @escaping CompletionGetEvent, _ result: ResponseGetEvent) {
